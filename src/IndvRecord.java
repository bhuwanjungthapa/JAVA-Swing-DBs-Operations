        ...
        if (actionEvent.getSource() == btnsearch) {
            try {
                ...
                if (selected.equals("Roll")) {
                    preparedStatement = dbConnection.con.prepareStatement("SELECT * FROM studenttbl WHERE rollno = ?");
                    preparedStatement.setString(1, txtfield.getText());
                } else if (selected.equals("Name")) {
                    preparedStatement = dbConnection.con.prepareStatement("SELECT * FROM studenttbl WHERE fullname = ?");
                    preparedStatement.setString(1, txtfield.getText());

                } else if (selected.equals("Level")) {
                    preparedStatement = dbConnection.con.prepareStatement("SELECT * FROM studenttbl WHERE level = ?");
                    preparedStatement.setString(1, txtfield.getText());

                } else if (selected.equals("Faculty")) {
                    preparedStatement = dbConnection.con.prepareStatement("SELECT * FROM studenttbl WHERE faculty = ?");
                    preparedStatement.setString(1, txtfield.getText());

                } else {
                    preparedStatement = dbConnection.con.prepareStatement("SELECT * FROM studenttbl WHERE semester = ?");
                    preparedStatement.setString(1, txtfield.getText());
